...
image.build.%: $(DPZ_COMPONENT_PREFIX_DIR)/Dockerfile $(if $(findstring 1,$(DPZ_ENABLE_BUILD_DEPENDENCY)),build.%)
	docker build $(DPZ_DOCKER_OPTIONS_EXTRA) --build-arg DPZ_VERSION=$(DPZ_VERSION) --build-arg DPZ_PROJECT=$(DPZ_PROJECT) --build-arg DPZ_COMPONENT=$(DPZ_COMPONENT) -f $< -t $(DPZ_DOCKER_FULL_IMAGE_NAME) $(if $(findstring 1,$(DPZ_GLOBAL_DOCKER_CONTEXT)),.,$(<D))


image.run.%: DPZ_COMPONENT=$*
image.run.%: image.build.%
	docker run $(DPZ_DOCKER_OPTIONS_EXTRA) --rm $(DPZ_DOCKER_FULL_IMAGE_NAME) $(DPZ_CMD)
