...
_deploy.k8s.action.%: _deploy.k8s.template.%.yml image.publish.%
	...
	@which md5sum > /dev/null || (echo md5sum is required for deployment; exit 1)
	@kubectl_cmd="kubectl --context k8s-$(DPZ_DEPLOY_ENV) -n $(K8S_NAMESPACE)"; \
	template_md5=$$(md5sum $< | awk '{print $$1'}); \
	stored_md5=$$($$kubectl_cmd get deployment/$(DPZ_COMPONENT) -o jsonpath='{.metadata.annotations.deployzor/template-md5}' 2> /dev/null); \
	if [ "$$template_md5" = "$$stored_md5" ]; \
	then \
	  echo "Deployment template md5 matches, will only update the image name"; \
	  $$kubectl_cmd set image deployment/$(DPZ_COMPONENT) $(DPZ_COMPONENT)=$(DPZ_DOCKER_FULL_IMAGE_NAME) --record; \
	  if (! timeout --preserve-status -k 10 120 $$kubectl_cmd rollout status deployment/$(DPZ_COMPONENT)); then echo Deployment failed; exit 1; fi; \
	else \
	  echo "Deployment template md5 does not match: deployment either does not exist or is out of date: will apply all over"; \
	  if (! grep "DPZ_DEPLOY_DOCKER_IMAGE" $< > /dev/null 2> /dev/null); then echo Deployment template missing DPZ_DEPLOY_DOCKER_IMAGE; exit 1; fi; \
	  sed -e 's|DPZ_DEPLOY_DOCKER_IMAGE|$(DPZ_DOCKER_FULL_IMAGE_NAME)|g' < $< > $@.yml; \
	  $$kubectl_cmd apply -f $@.yml; \
	  if (! timeout --preserve-status -k 10 120 $$kubectl_cmd rollout status deployment/$(DPZ_COMPONENT)); then echo Deployment failed; exit 1; fi; \
	  $$kubectl_cmd annotate --overwrite deployment $(DPZ_COMPONENT) deployzor/template-md5=$$template_md5; \
	fi
	@rm -f $@.yml
